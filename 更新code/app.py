...
from modules.audio_processor import (
    StreamingTTSManager,
    EdgeStreamingTTSManager,  # 使用免费的 edge-TTS
    transcribe_file,
)
...
if "resume_analysis" not in st.session_state:
    st.session_state.resume_analysis = None  # 存储解析后的 JSON
if "resume_file_name" not in st.session_state:
    st.session_state.resume_file_name = ""  # 存储文件名
# IDE 相关状态
if "ide_visible" not in st.session_state:
    st.session_state.ide_visible = False  # IDE 是否显示
if "is_algorithm_mode" not in st.session_state:
    st.session_state.is_algorithm_mode = False  # 是否算法题模式
if "submitted_codes" not in st.session_state:
    st.session_state.submitted_codes = []  # 历史提交记录
if "current_code" not in st.session_state:
    st.session_state.current_code = ""  # 当前编辑器中的代码
if "code_ready_to_submit" not in st.session_state:
    st.session_state.code_ready_to_submit = False  # 代码是否准备提交
if "ide_audio_submitted" not in st.session_state:
    st.session_state.ide_audio_submitted = False  # IDE 语音是否已提交
...
with st.sidebar:
    ...
    if st.session_state.resume_uploaded:
        st.success(f"✅ 已上传：{st.session_state.resume_file_name}")
        st.caption("简历将用于个性化面试")
        ...
        st.info("上传 PDF 简历，让 AI 面试官更了解你")
        ...
        if uploaded_file is not None:
            ...
            try:
                with progress_placeholder:
                    st.progress(0.2)
                status_placeholder.info("⏳ 步骤 1/2: 正在提取 PDF 文本...")
                
                with progress_placeholder:
                    st.progress(0.4)
                status_placeholder.info("⏳ 步骤 2/2: AI 正在深度分析简历（约 10-15 秒）...")
                ...
                with progress_placeholder:
                    st.progress(1.0)
                status_placeholder.success("✅ 简历解析完成！")
                ...
                with progress_placeholder:
                    st.progress(0.0)
                status_placeholder.error(f"❌ 简历解析失败：{str(e)}")
                # 删除临时文件
                temp_pdf_path.unlink(missing_ok=True)
    ...
    # 编辑器开关
    st.subheader("📝 代码编辑器")
    if st.button("✏️ 打开/关闭编辑器", use_container_width=True, 
                 type="primary" if st.session_state.ide_visible else "secondary"):
        st.session_state.ide_visible = not st.session_state.ide_visible
        st.session_state.is_algorithm_mode = st.session_state.ide_visible
        st.rerun()
    
    if st.session_state.ide_visible:
        st.success("✅ 编辑器已打开\n\n可以在右侧编写代码")
        if st.session_state.submitted_codes:
            st.caption(f"📝 已提交 {len(st.session_state.submitted_codes)} 次代码")
    else:
        st.info("💡 需要写代码时再打开")
...
with tab_voice:
    ...
    recent = st.session_state.history[-2:] if st.session_state.history else []
    if recent:
        st.markdown("**最近对话**")
        for msg in recent:
    ...
    if not _history:
        ...
        if st.session_state.report_generating:
            ...
            with st.spinner("Qwen-max 正在深度分析面试表现，请稍候（约 15~30 秒）..."):
                try:
                    # 传入简历分析结果 ⭐
                    resume_analysis = st.session_state.get("resume_analysis", None)
                    for partial_report in ai_report_stream(_history, resume_analysis=resume_analysis):
...
if user_input:
    ...
    with st.spinner("面试官正在思考..."):
        try:
            ...
            if st.session_state.enable_tts:
                # 🟢 使用免费的 edge-TTS（延迟更低，无需 API Key）
                streaming_tts = EdgeStreamingTTSManager()
                streaming_tts.start()
    ...
    if st.session_state.enable_tts and st.session_state.streaming_tts:
        ...
        # 🚀 关键优化：等待所有句子都生成完成（使用计数器而非仅检查队列）
        start_time = time.time()
        max_wait = 60  # 最多等待 60 秒
        
        # 获取需要处理的句子总数
        total_sentences = st.session_state.streaming_tts.get_total_sentences()
        print(f"📊 共需处理 {total_sentences} 句")
        ...
        while time.time() - start_time < max_wait:
            ...
            # 🚀 关键修改：检查是否所有句子都处理完毕（使用计数器）
            completed_count = st.session_state.streaming_tts.get_completed_count()
            current_collected = len(st.session_state.temp_audio_buffer)
            
            # 更新进度
            progress_placeholder.info(f"⏳ 已生成 {completed_count}/{total_sentences} 句语音（已收集 {current_collected} 句）...")
            
            # 🚀 核心判断：所有句子都已完成处理
            if st.session_state.streaming_tts.is_all_completed():
                # 所有句子处理完毕，再等待 0.5 秒确保最后的结果都放入队列
                time.sleep(0.5)
                # 最终收集
                try:
                    while True:
                    ...
                    pass
                print(f"✅ 所有 {total_sentences} 句处理完毕，已收集 {len(st.session_state.temp_audio_buffer)} 句")
                break
